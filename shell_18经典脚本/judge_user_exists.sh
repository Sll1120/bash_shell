...
# * Create time : 2022-09-30 11:01
# * Filename : judge_user_exists.sh
function failure() {
...
# 运行展示
# 请输入需要创建的用户名,输入后按Enter继续:
# 您必须输入需要创建的用户名！
# 请输入需要创建的用户名,输入后按Enter继续:root
# 您输入用户名为:root,请输入[y/n]确认:y
# 您输入的用户已存在，该用户名不可用，请核对后重新输入！
# 请输入需要创建的用户名,输入后按Enter继续:freeops
# 您输入用户名为:freeops,请输入[y/n]确认:y
# 您输入需要创建的用户名为：freeops
